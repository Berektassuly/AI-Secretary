...
> Mission-critical система для автоматизации протоколирования встреч: транскрибация локальным Whisper (whisper.cpp), извлечение задач и их публикация в Jira.
...
[Браузер]
   ↓
[Next.js App Router]
   ├── POST /api/whisper  ──▶  локальный whisper.cpp
...
3. Заполните значения переменных окружения:
   - `WHISPER_CPP_BINARY` и `WHISPER_CPP_MODEL` — пути до собранного whisper.cpp и GGML-модели.
...
> 💡 Совет: скачайте актуальный релиз [`whisper.cpp`](https://github.com/ggerganov/whisper.cpp) и соответствующую GGML‑модель. Если на сервере есть GPU, собирайте бинарь с `WHISPER_CUBLAS=1`, чтобы транскрибация выполнялась на видеокарте.
...
- **API Routes**
- `POST /api/whisper` — принимает файл (FormData `file`), проверяет лимит 25 МБ и запускает локальный `whisper.cpp` (CPU/GPU, в зависимости от того, как собран бинарь). При отсутствии настроенного движка возвращает понятную ошибку.
...
| ---------- | ---------- |
| `WHISPER_CPP_BINARY` | Путь до исполняемого файла `whisper.cpp` (обязателен). |
| `WHISPER_CPP_MODEL` | Путь до GGML-модели для `whisper.cpp` (обязателен). |
...
- Ошибки: `400` (нет файла), `413` (лимит), `502` (ошибка локального движка Whisper).
